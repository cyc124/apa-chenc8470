...
public class ElevensBoard extends Board {
	...
	public boolean anotherPlayIsPossible() {
		...
		for (int i=0; i<indexes.size()-1;i++)
		{
			for (int j=i+1; j<indexes.size();j++)
			{
				...
				{
					return containsPairSum11(selected);
				}
			}
		}
	...
	if (selectedCards.size()==3)
		{
			if (cardAt(selectedCards.get(0)).rank()!=cardAt(selectedCards.get(1)).rank()&&cardAt(selectedCards.get(0)).rank()!=cardAt(selectedCards.get(2)).rank()&&cardAt(selectedCards.get(1)).rank()!=cardAt(selectedCards.get(2)).rank())
			{
